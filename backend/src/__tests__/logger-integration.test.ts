...
describe('Logger Integration with tRPC', () => {
  ...
  describe('tRPC procedure logging', () => {
    ...
    it('should maintain request ID throughout procedure lifecycle', async () => {
      ...
      expect(context.log.info).toHaveBeenNthCalledWith(3, 'Procedure completed');
    });
  });
  ...
  describe('Request tracing scenarios', () => {
    it('should enable easy request tracing with multiple operations', async () => {
      ...
      expect(context.log.info).toHaveBeenNthCalledWith(1, 'Authentication started', { email: 'test@example.com' });
      expect(context.log.info).toHaveBeenNthCalledWith(2, 'User validated successfully', { userId: 123 });
      expect(context.log.info).toHaveBeenNthCalledWith(3, 'Database query initiated', { table: 'users' });
      expect(context.log.info).toHaveBeenNthCalledWith(4, 'Response prepared', { statusCode: 200 });
    });
    ...
    it('should handle concurrent requests with different IDs', async () => {
      ...
      const context1 = await createContext({ req: mockReq, env: env, resHeaders: mockResHeaders });
      const context2 = await createContext({ req: mockReq, env: env, resHeaders: mockResHeaders });
